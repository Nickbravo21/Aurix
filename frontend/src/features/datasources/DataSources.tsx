...
export default function DataSources() {
  const [connectedSources] = useState<any[]>([])

  const availableSources = [
    { name: 'Google Sheets', type: 'Spreadsheet', icon: '📊', available: true },
    { name: 'QuickBooks', type: 'Accounting', icon: '📗', available: false, comingSoon: true },
    { name: 'Stripe', type: 'Payment', icon: '💳', available: false, comingSoon: true },
    { name: 'Slack', type: 'Notifications', icon: '💬', available: false, comingSoon: true },
  ]
          ...
          <div className="space-y-4">
            {connectedSources.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-lg text-slate-300 font-semibold mb-2">No data sources connected</p>
                <p className="text-sm text-slate-400">Connect Google Sheets below to start analyzing your data</p>
              </div>
            ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {availableSources.map((source) => (
              <button
                ...
                className={`p-6 border rounded-lg transition-all duration-300 group text-left relative ${
                  source.available
                    ? 'bg-slate-800/50 hover:bg-slate-800 border-slate-700 hover:border-cyan-500/50 cursor-pointer'
              ...
              >
                <div className="flex items-center gap-4 mb-3">
                  <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-2xl transition-all duration-300 ${
                    source.available
                      ? 'bg-slate-700 group-hover:bg-gradient-to-br group-hover:from-cyan-500 group-hover:to-blue-600'
                      : 'bg-slate-800'
                  }`}>
                    {source.icon}
                  </div>
